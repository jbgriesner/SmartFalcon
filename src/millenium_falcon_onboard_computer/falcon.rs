...
pub fn generate_paths(galaxy: &Galaxy, src: String, dst: String, cd: u32, autonomy: u32) -> Vec<Path> {
    ...
    fn visit(time: u32, src: u32, dst: u32, galaxy: &Galaxy, path: &mut Path, all_paths: &mut Vec<Path>, seen: &mut HashMap<(u32,u32),bool>, cd: u32, autonomy: u32) {
        ...
        if time <= cd {
            if src == dst {
                ...
                for (planet, distance) in galaxy.planets.get(&src).unwrap().neighbors.iter() {
                    ...
                    println!("neighbors: {:?}", galaxy.planets.get(&src).unwrap().neighbors);
                    ...
                    if !seen.get(&(*planet,time)).unwrap() && *distance <= autonomy {
                        if *planet == src {
                            visit(time, *planet, dst, galaxy, path, all_paths, seen, cd, autonomy);
                        } else {
                            visit(time, *planet, dst, galaxy, path, all_paths, seen, cd, autonomy - distance);
                        }
                    }
                }
            }
    ...
    visit(time, *src_id, *dst_id, galaxy, &mut path, &mut all_paths, &mut seen_planets, cd, autonomy);
